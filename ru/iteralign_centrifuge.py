...
def url_list_generation(args, difference_set):
    ...
    summary_centrifuge = pd.read_csv(
        args.csummary,
        sep="\t",
    )
    ...
    url_list = list(
        summary_centrifuge[summary_centrifuge["taxid"].isin(difference_set)][
            "fasta_path"
        ]
    )
...
def download_references(args, url_list, taxid_set):
    ...
    for link in url_list:
        ...
        with gzip.open(compressed_file, "rt") as fh, gzip.open(
            args.path + args.prefix + args.gfasta, "at"
        ) as fasta_seq:
    ...
    if args.plasmids:
        r = ("name", "path")
        logging.info(
            "Obtaining the plasmids for the following taxids: {}".format(taxid_set)
        )
        with open(args.csummary) as f:
            ...
            for taxid in taxid_set:
                with gzip.open(args.plasmids, "rt") as fh, gzip.open(
                    args.path + args.prefix + args.gfasta, "at"
                ) as fasta_seq:
                    for seq_record in SeqIO.parse(fh, "fasta"):
                        if (
                            d[taxid]["name"] in seq_record.description
                            and len(seq_record) > args.seqlength
                        ):
                            lengths[seq_record.id] = len(seq_record)
                            SeqIO.write(seq_record, fasta_seq, "fasta")
...
def generate_mmi(args, counter):
    args.toml["conditions"]["reference"] = os.path.abspath(
        "{}{}_{}_{}".format(args.path, args.prefix, counter, args.mindex)
    )
    minimap_cmd = [
        "minimap2",
        "-x",
...
def parse_fastq_file(
    ...
    with open(os.devnull, "w") as devnull:
        ...
        centrifuge_cmd = "centrifuge -p {} -x {} -q {}".format(
            args.threads, args.cindex, fastq_str
        )
        ...
        logging.info(
            "Post centrifuge run: {} seconds".format(
                time.time() - centrifuge_start_time
            )
        )
        ...
        out_df = pd.read_csv(StringIO(out), sep="\t", usecols=output_fields)
        ...
        all_reads_df = pd.read_csv(args.path + args.prefix + args.readfile, sep="\t")
        ...
        taxid_result_set = set(
            taxid_count[taxid_count["taxID"].ge(args.threshold)].index.values.tolist()
        )
        ...
        if taxid_result_set:
            ...
            if difference_set:
                ...
                update_message = "Updated the minimap MMI to {}".format(
                    args.toml["conditions"]["reference"]
                )
...
class FastqHandler(FileSystemEventHandler):
    ...
    def processfiles(self):
        ...
        if self.args.references:
            logging.info(
                "References argument provided. Will download references genomes."
            )
            ...
            self.length_dict.update(
                download_references(self.args, self.url_list, self.downloaded_set)
            )
