"""ru_barcode.py
This is a temporary proof-of-concept script for interacting with basecalled data.
Specifically, we are looking at barcodes.

For this script to work we require the `barcode_kits` field to set in the 
`caller_settings` table e.g:
...
def simple_analysis(
    ...
    validate_TOML : bool
        Validate the TOML file when reloading the live file
    run_info : dict
    ...
    while client.is_running:
        ...
        if r > 0:
            s1 = "{}R/{:.5f}s {}"
            logger.info(s1.format(r, t1 - t0, barcode_counter))
...
def run(parser, args):
    ...
    run_info, conditions, reference, caller_kwargs = get_run_info(
        args.toml,
        num_channels=512,
        validate=args.develop,
    )
    live_toml = Path("{}_live".format(args.toml))

    # Check for barcode kits here, in the future this should be done using
    #   TOML validation
    if "barcode_kits" not in caller_kwargs:
        sys.exit("'barcode_kits' not in TOML file")
    if not isinstance(caller_kwargs.get("barcode_kits"), list):
        sys.exit("'barcode_kits' should be an array")
